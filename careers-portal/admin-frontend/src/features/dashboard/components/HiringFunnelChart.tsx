...
import type { HiringFunnelStage } from '@/features/dashboard/types'
...
export function HiringFunnelChart({ data }: HiringFunnelChartProps) {
  ...
  if (data.length === 0) {
    return <ChartEmptyState message="No pipeline activity yet" height={300} />
  }
      ...
      <BarChart data={data} layout="vertical" margin={{ top: 4, right: 28, left: 8, bottom: 4 }}>
        ...
        <Tooltip
          ...
          content={({ active, payload }) => {
            ...
            const point = payload[0].payload as HiringFunnelStage
